...
# RNIB Best Practice in Accessible Gaming 2025 [BPAG2025]
...
Narration as defined above should be adjustable in terms of its speed.  After all, just like we all read at different paces, we also all listen and comprehend at different rates.  If a player needs information quickly and can understand the speech at a much faster rate, they should be able to speed up the rate at which the narration is being spoken. Many screen reader users will listen to speech at over 300 words per minute [^1] and sometimes as fast as 500 words per minute. With the range of variability between users it’s best to offer as much granularity in speeds as is practical. Examples of games with adjustable speech rates are Diablo IV, Sea Of Thieves and any game that has screen reader compatibility on PC
...
Accessing menu and interface elements using free moving cursers can cause problems for people with sight loss or dexterity issues. Blind users have no context for where a free moving curser is until it hits an interactive element (and that’s only if the element has an audible or tactile cue). If your game is accessible through magnification but there are no visual landmarks near to the curser to tell users where they are then users with high magnification settings may get lost. Users with dexterity issues or non-standard input devices may also have trouble with the precision of landing on an element.
...
### 8.3	Aiming 
If your game requires use of a reticle for aiming then consider how to allow players with no sight to aim with freedom and know what they’re targeting. Audio-based aiming provides the gamers with sound cues to guide them to the enemy without snapping to the target. Auto aiming locks on to the nearest target which reduces agency but can be useful for players with dexterity issues or who are using non-standard interfaces. Ideally audio-based aiming should be the default with auto aiming available from a menu setting.  

An example of an audio-based aiming system can be seen in Sea Of Thieves or Atomfall in the form of aim assist audio cues. These allow a player to line up a target in the sights of their guns and fire when the audio indicates the target is in position. 
...
The aim of making a game accessible should be to enable a blind and partially sighted person to play it completely independently. The main aim, however, is to enable a blind and partially sighted player to enjoy the game and allowing cooperative play can do this. This could be where different members of a team take on different tasks such as in Sea of Thieves or where two players could control the same character with a sighted player handling the tasks that require vision such as through the co-pilot system built into Horizon Forbidden West. 
...
Mechanics: [Idle gaming](#1055-idle-gaming), [Minigames](#1058-minigames), [Powerups](#10513powerups), [Story](#10519story)
...
Mechanics: [Cards](#1052-cards), [Inserting to grid](#1056-inserting-to-grid), [Multiplayer](#1059-multiplayer), [Resource management](#10515resource-management), [Timed gameplay](#10521timed-gameplay)
...
Mechanics: [Feature unlock](#1053-feature-unlock), [Guilds](#1054-guilds), [Merging](#1057-merging), [Minigames](#1058-minigames), [Placing items](#10512placing-items), [Raid parties](#10516raid-parties), [Research](#10514research), [Resource management](#10515resource-management), [Single player combat](#10518single-player-combat), [Upgrade building](#10522upgrade-building)
...
Mechanics: [Merging](#1057-merging)
...
Mechanics: [Inserting to grid](#1056-inserting-to-grid)
...
Mechanics: [Pattern matching](#10511pattern-matching), [Powerups](#10513powerups), [Story](#10519story), [Timed gameplay](#10521timed-gameplay)
...
Mechanics: [Pattern matching](#10511pattern-matching), [Placing items](#10512placing-items), [Inserting to grid](#1056-inserting-to-grid), [Timed gameplay](#10521timed-gameplay)
...
Mechanics: [Powerups](#10513powerups), [Rolling](#10517rolling), [Timed gameplay](#10521timed-gameplay) 
...
Mechanics: [Placing items](#10512placing-items), [Resource management](#10515resource-management), [Story](#10519story)
...
Mechanics: [Guilds](#1054-guilds), [Placing items](#10512placing-items), [Research](#10514research), [Single player combat](#10518single-player-combat), [Strategy](#10520strategy), [Timed gameplay](#10521timed-gameplay), 
...
Mechanics: [Placing items](#10512placing-items),[Timed gameplay](#10521timed-gameplay), [Story](#10519story), [Strategy](#10520strategy)
...
Mechanics: [Inserting to grid](#1056-inserting-to-grid), [Multiplayer](#1059-multiplayer), [Timed gameplay](#10521timed-gameplay)
...
[Cycle through Elements](#1064-cycle-through-elements), [Grid Navigation](#1066-grid-navigation), [List view](#1067-list-view)

#### 10.5.3	 Feature unlock 
Description:
...
Accessibility Interactions:
[Alert Popups](#1061-alert-popups)
...
Accessibility Interactions:
[Explore by Touch](#1065-explore-by-touch), [Grid Navigation](#1066-grid-navigation)
...
Accessibility Interactions:
[Explore by Touch](#1065-explore-by-touch), [Grid Navigation](#1066-grid-navigation), [List view](#1067-list-view)
...
Accessibility Interactions:
[Colour Coding ](#1063-colour-coding), [Explore by Touch](#1065-explore-by-touch), [Grid Navigation](#1066-grid-navigation), [List view](#1067-list-view)

#### 10.5.12	Placing items 
Description:
...
Accessibility interactions:
[Alert Popups](#1061-alert-popups), [List view](#1067-list-view)

#### 10.5.15	Resource management 
Description:
...
Accessibility Interactions:
[Beacons](#1062-beacons), [Colour Coding ](#1063-colour-coding)

#### 10.5.18	Single player combat 
Description:
...
Accessibility Interactions:
[Colour Coding ](#1063-colour-coding), [Cycle through Elements](#1064-cycle-through-elements), [List view](#1067-list-view) 

#### 10.5.21	Timed gameplay 
Description:
